 ...
 */
public final class ConstantFloatMultivalueBlock extends AbstractNonThreadSafeRefCounted implements FloatBlock {

    private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(ConstantFloatMultivalueBlock.class);

    private final FloatVector values;
     ...
     */
    ConstantFloatMultivalueBlock(FloatVector values, int positionCount) {
    ...
    public float getFloat(int valueIndex) {
        return values.getFloat(valueIndex);
    }
        ...
        try (var builder = blockFactory().newFloatBlockBuilder(positions.length)) {
            for (int pos : positions) {
                ...
                for (int v = 0; v < values.getPositionCount(); v++) {
                    builder.appendFloat(values.getFloat(v));
                }
    ...
    public FloatBlock expand() {
        ...
        try (FloatVector.Builder builder = blockFactory().newFloatVectorBuilder(expandedPositionCount)) {
    ...
    public FloatBlock deepCopy(BlockFactory blockFactory) {
        return blockFactory.newConstantFloatMultivalueBlockWith(values.deepCopy(blockFactory), positionCount);
    }
