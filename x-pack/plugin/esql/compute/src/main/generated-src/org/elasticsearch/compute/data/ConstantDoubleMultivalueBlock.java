 ...
 */
public final class ConstantDoubleMultivalueBlock extends AbstractNonThreadSafeRefCounted implements DoubleBlock {

    private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(ConstantDoubleMultivalueBlock.class);
     ...
     */
    ConstantDoubleMultivalueBlock(DoubleVector values, int positionCount) {
        ...
        try (var builder = blockFactory().newDoubleBlockBuilder(positions.length)) {
            for (int pos : positions) {
                ...
                for (int v = 0; v < values.getPositionCount(); v++) {
                    builder.appendDouble(values.getDouble(v));
                }
    ...
    public DoubleBlock expand() {
        ...
        try (DoubleVector.Builder builder = blockFactory().newDoubleVectorBuilder(expandedPositionCount)) {
    ...
    public DoubleBlock deepCopy(BlockFactory blockFactory) {
        return blockFactory.newConstantDoubleMultivalueBlockWith(values.deepCopy(blockFactory), positionCount);
    }
